...
public class Imdb {
    ...
    public static class ReadTitles extends Mapper<Object, Text, Text, Text> {
        @Override
        public void map ( Object key, Text value, Context context )
            ...
            String title_id = attr[0];
            String title_type = attr[1].toString();
            String name = attr[2];
            String year = attr[3];
            String genres = attr[4];
                 	            
            line = attr[4].toString();
            ...
            String tag = title_type + ";" + name + ";" + year + ";" + genre;
            
            if ((title_type.equals("movie") || title_type.equals("tvSeries") || title_type.equals("tvMovie")) && (genre.contains("Comedy") || genre.contains("Romance") || genre.contains("Action")))
                context.write(new Text(title_id), new Text("title    " + tag));
        }
        }
    ...
    public static class ReadActors extends Mapper<Object, Text, Text, Text> {
        @Override
        public void map ( Object key, Text value, Context context )
                        throws IOException, InterruptedException {
            String line = value.toString();
    ...
    public static class ReadDirectors extends Mapper<Object, Text, Text, Text> {
        @Override
        public void map ( Object key, Text value, Context context )
                        throws IOException, InterruptedException {
            String line = value.toString();
...
/*    // count number of each gener type movie released for each year from 2000
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(Imdb1.class);
        job.setNumReduceTasks(1);
        MultipleInputs.addInputPath(job,new Path(args[0]),TextInputFormat.class,ReadTitles.class);
        MultipleInputs.addInputPath(job,new Path(args[1]),TextInputFormat.class,ReadActors.class);
        MultipleInputs.addInputPath(job,new Path(args[2]),TextInputFormat.class,ReadDirectors.class);
